...
def preprocess_hydra_scan_result(hydra_scan_result):
...
def generate_hydra_table(data, header_style, cell_style):
...
def generate_pdf_report(filename, toolname, subnet_scanned=None, target_discovered=None, cve_discovered=None, hydra_scan_result=None):
    try:
        ...
        # Add logo
        logo_path = os.path.join(current_dir, "..", "app", "resource", "images", "logo.png")  # Replace with the actual path to your logo
        ...
        if target_discovered:
            ...
            target_table = generate_target_table(target_discovered, table_header_style, table_cell_style)
        ...
        if cve_discovered:
            ...
            # Generate CVE table
            cve_table = generate_cve_table(cve_discovered, table_header_style, table_cell_style)
        ...
        if hydra_scan_result:
            ...
            hydra_table_data = preprocess_hydra_scan_result(hydra_scan_result)
            if hydra_table_data:
                content.append(Paragraph("<b>Résultat de l'attaque Hydra :</b>", table_header_style))
                hydra_table = generate_hydra_table(hydra_table_data, table_header_style, table_cell_style)
...
def generate_cve_table(cve_data, header_style, cell_style):
...
def generate_target_table(target_data, header_style, cell_style):
...
json_file_path = os.path.join(current_dir, "..", "app", "profiles", "Test.kgb")

# Read data from JSON file
data = read_json(json_file_path)

# Extract relevant information
toolname, subnet_scanned, target_discovered, cve_discovered, hydra_scan_result = extract_info(data)

# Path to save the PDF report
pdf_file_path = os.path.join(current_dir, "cybersecurity_report.pdf")

# Generate PDF report
generate_pdf_report(pdf_file_path, toolname, subnet_scanned, target_discovered, cve_discovered, hydra_scan_result)
